...
alphanumeric = RegexValidator(r'^[0-9a-zA-Z\_]*$', '只允许使用中文、字母和数字')
...
def user_info(request, user_id):
    ...
    return render_to_response('account/user-info.html', {'request': request, 'title': "用户信息",
                                                 'user': u, 'conf': conf,
...
def reg(request):
    if request.method == 'GET':
        return render_to_response('account/reg.html', {'conf': conf, 'title':"注册"},
...
def user_login(request):
    if request.method == 'GET':
        return render_to_response('account/login.html', {'conf': conf, 'title':"登录"},
                                  context_instance=RequestContext(request))
    elif request.method == 'POST':
        username = request.POST['username']
        password = request.POST['password']
...
def setting(request):
    if request.method == 'GET':
        ...
        request.user.profile.weibo = request.POST['weibo']
        request.user.email = request.POST['email']
        request.user.profile.institute = request.POST['institute']
        request.user.profile.qnumber = request.POST['qnumber']
        request.user.profile.about = request.POST['about']
        request.user.profile.save()
        request.user.save()
        return render(request, 'account/user-setting.html', locals())
...
def view_mention(request):
    ...
    for m in new:
        m.read = True
        m.save()
    return render_to_response('account/user-mention.html', {'request': request, 'title':"提醒",
                                                    'conf': conf,
                                                    'new': new,
                                                    'old': old, },
                              context_instance=RequestContext(request))
...
def change_password(request):
    u = request.user
    if request.method == 'GET':
        return render_to_response('account/change-password.html', {'request': request, 'title':"修改密码",
                                                           'conf': conf},
                                  context_instance=RequestContext(request))
    elif request.method == 'POST':
        ...
        if authenticate(username=u.username, password=old):
            u.set_password(new)
            u.save()
            messages.add_message(request, messages.SUCCESS, "密码成功更新")
            return HttpResponseRedirect(reverse('change_password'))
        else:
            messages.add_message(request, messages.WARNING, "当前密码不正确")
            return HttpResponseRedirect(reverse('change_password'))
...
def user_avatar(request):
    u = request.user
    if request.method == 'GET':
        return render_to_response('account/user-avatar.html', {'request': request, 'title':"头像设置",
                                                       'conf': conf},
                                  context_instance=RequestContext(request))
    else:
